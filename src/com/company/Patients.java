...
public class Patients {
	public static void Table_Patients() {
		...
		do{
			...
			System.out.println("\n\t1) Press to update table patients\n \t2) Press to add to table patients \n\t"
								+ "3) Press to remove from table patients\n\t4) Press to close\n\t0) Press to Back") ;
			System.out.print( "\n(@user)--> ");
			call =scan.nextInt();
			if ( call ==1){
				...
				try {
					...
					String query = "UPDATE patients SET F_name = ?, L_name = ?, Phone = ? where ID_patient = ? ";
					...
					System.out.print("Enter ID_patient (@user)--> ");
					_int1 =scan.nextInt();
					_statement.setInt(4,_int1);
				...
				try {
					Connection connection = DriverManager.getConnection(URL,name,password);
					System.out.println("Connect ");
					String query = "INSERT INTO patients (ID_patient,F_name,L_name,Phone)VALUES(?,?,?,?)" ;
					PreparedStatement _statement = connection.prepareStatement(query);
